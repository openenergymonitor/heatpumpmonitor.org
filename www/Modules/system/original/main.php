...
<div id="app">
	<div style=" background-color:#f0f0f0; padding-top:20px; padding-bottom:10px">
		<div class="container-fluid">
			...
			<div style="float:right; margin-right:10px" v-if="show_when_running_option">
				<div class="input-group">
					<span class="input-group-text">When running</span>
					<span class="input-group-text"><input type="checkbox" v-model="show_when_running"></span>
				</div>
			</div>
	...
	<div class="container-fluid mt-3">
		<table width="100%" class="table-hpmon">
			<thead>
				<tr>
					<th class="header">&nbsp;</td>
					<th v-if="!show_kwh_m2" class="header" colspan="5">Property</th>
					<th v-if="show_kwh_m2" class="header" colspan="3">Property</th>
				...
				<tr>
					...
					<!-- Heating System -->
					<th @click="sort('hp_model', 'asc')" class="lborder">Make / Model</th>
					...
					<!-- Stats when running -->
					<th @click="sort('standby_kwh', 'desc')" class="lborder right" v-if="show_when_running">Standby</th>
					<th @click="sort('when_running_elec_kwh', 'desc')" class="right" v-if="show_when_running">Electric</th>
					<th @click="sort('when_running_heat_kwh', 'desc')" class="right" v-if="show_when_running">Heat</th>
					<th @click="sort('when_running_cop', 'desc')" class="center" v-if="show_when_running">COP</th>
					<th @click="sort('when_running_flowT', 'desc')" class="center" v-if="show_when_running">Flow</th>
					<th @click="sort('when_running_flow_minus_return', 'desc')" class="center" v-if="show_when_running">Delta</th>
					<th @click="sort('when_running_outsideT', 'desc')" class="center" v-if="show_when_running">Outside</th>

					<!-- Performance -->
					<th @click="sort('elec_kwh', 'desc')" class="lborder right">Electric</th>
			...
			<tbody>
				<template v-for="n in sortedNodes">
					<tr v-bind:id="'row'+n.id" v-bind:class="hiliteClass(n)">
						...
						<!-- Heating System -->
						<td class="lborder" v-bind:title="n.hp_model">{{n.hp_model.replace(/ \(.*\)/, ' &mldr;')}}</td>
						...
						<!-- Stats when running -->
						<td v-bind:class="monthClass(n) + ' nowrap lborder'" align="right" v-if="show_when_running">
						...
						<td v-bind:class="monthClass(n) + ' nowrap'" align="right" v-if="show_when_running">
						{{unit_dp(n.when_running_cop, '', 1)}}</td>
						...
						<!-- Performance -->
						<td v-bind:class="sinceClass(n) + ' nowrap lborder'" v-bind:title="sinceDate(n)" align="right">
