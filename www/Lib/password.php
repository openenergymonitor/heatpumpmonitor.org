...
// (www/Lib/password.php). Both sites authenticate against the same emoncms
// `users` table, so a password written by one has to be readable by the other:
// the moment these two copies disagree about how a hash is produced or checked,
// accounts stop being able to log in to one of the sites. Change it in one
// place, copy it to the other, and check with scripts/check_shared.sh.
//
// It depends on nothing but $settings, so it can be dropped into any codebase
// that has a $settings['password'] block.
// ---------------------------------------------------------------------------------------------------------
 ...
 * can spare the RAM, which on emoncms.org means both.
 *
